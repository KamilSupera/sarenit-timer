...
**No long-lived credentials exist anywhere.** GitHub Actions authenticates to AWS with OIDC. The role's trust policy is pinned to `repo:<owner>/<repo>:ref:refs/heads/main`, so a fork — or any other branch — cannot assume it, and its IAM policy grants exactly three things: write objects into the one site bucket, list that bucket, and invalidate the one distribution. Nothing else in the account is reachable from CI.
...
- **Environments → `production`**: add required reviewers and restrict it to the `main` branch
